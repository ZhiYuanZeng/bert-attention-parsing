...
from utils.parse_utils import evalb, MRG, MRG_labeled, comp_tree, flat_tree
...
from model.parser import parse_cyk,parse_greedy
...
def unsupervised_parsing(args, model, tokenizer, prefix=""):
    ...
    for i, batch in enumerate(tqdm(eval_dataloader, desc="parsing")):
        ...
        with torch.no_grad():
            ...
            # evaluate
            for j,(pred_tree, tgt_tree, nltk_tree) in enumerate(zip(pred_trees, tgt_trees, nltk_trees)):
...
def parse_args():
    ...
    parser.add_argument('--remove_bpe', action='store_true', help='whether to remove bpe from attention matrix or hiddens') 
    parser.add_argument('--multi-lingual', action='store_true', help='whether to use multi-lingual BERT') # NOTE: if use mBERT, tokenizer also changed, which means data changed
